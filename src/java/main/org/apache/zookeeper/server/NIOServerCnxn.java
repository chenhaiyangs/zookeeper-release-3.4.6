 ...
 * NIOServerCNxnFactory创建的一个抽象的连接NIOServerCnxn，管理连接的生命周期
 * NIOServerCnxn是如何解决拆包粘包问题的？
 */
public class NIOServerCnxn extends ServerCnxn {
    ...
    /**
     * 构建NIOServerCnxn
     * @param zk ZookeeperServer
     * @param sock socket连接
     * @param sk NIO selectKey
     * @param factory 工厂
    ...
    public void sendBuffer(ByteBuffer bb) {
        try {
            //如果通道可写，则直接写进socket，否则，写进outgoingBuffers的缓冲区
            if (bb != ServerCnxnFactory.closeConn) {
    ...
    void doIO(SelectionKey k) throws InterruptedException {
        try {
            ...
            if (k.isReadable()) {
                ...
                //read() 方法返回的 int 值表示读了多少字节进 Buffer 里
                ...
                /*
                 * 只有incomingBuffer.remaining() == 0,才会进行下一步的处理,
                 * 否则一直读取数据直到incomingBuffer读满,此时有两种可能:
                ...
                if (outgoingBuffers.size() > 0) {
                    ...
                    for (ByteBuffer b : outgoingBuffers) {
                        ...
                        if (directBuffer.remaining() < b.remaining()) {
                             ...
                             * slice返回原ByteBuffer中的可写区
                             * 分片，只获取能刚刚能写满directBuffer的那部分
                             */
     ...
     * 读取请求长度的字节数，并分配真正数据长度的incomingBuffer用于接下来接收数据
     */
    ...
    private void closeSock() {
        ...
        LOG.info("Closed socket connection for client " + sock.socket().getRemoteSocketAddress() + (sessionId != 0 ?
